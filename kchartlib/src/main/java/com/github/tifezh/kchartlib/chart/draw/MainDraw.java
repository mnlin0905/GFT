...
import com.github.tifezh.kchartlib.chart.CommonChartUpDownModeColor;
...
public abstract class MainDraw implements IChartDraw<ICandle>,CommonChartUpDownModeColor {
    ...
    public MainDraw(BaseKChartView view) {
        ...
        mContext=context;
        mUpPaint.setColor(ContextCompat.getColor(context,R.color.chart_red));
        mDownPaint.setColor(ContextCompat.getColor(context,R.color.chart_green));
    }
    ...
    private void drawCandle(BaseKChartView view, Canvas canvas, float x, float high, float low, float open, float close) {
        ...
        if (open > close) {
            //实心
            if(mCandleSolid) {
                canvas.drawRect(x - r, close, x + r, open, mUpPaint);
                canvas.drawRect(x - lineR, high, x + lineR, low, mUpPaint);
            }
            else {
                mUpPaint.setStrokeWidth(mCandleLineWidth);
                canvas.drawLine(x, high, x, close, mUpPaint);
                canvas.drawLine(x, open, x, low, mUpPaint);
                canvas.drawLine(x - r + lineR, open, x - r + lineR, close, mUpPaint);
                canvas.drawLine(x + r - lineR, open, x + r - lineR, close, mUpPaint);
                mUpPaint.setStrokeWidth(mCandleLineWidth * view.getScaleX());
                canvas.drawLine(x - r, open, x + r, open, mUpPaint);
                canvas.drawLine(x - r, close, x + r, close, mUpPaint);
            }

        } else if (open < close) {
            canvas.drawRect(x - r, open, x + r, close, mDownPaint);
            canvas.drawRect(x - lineR, high, x + lineR, low, mDownPaint);
        } else {
            canvas.drawRect(x - r, open, x + r, close + 1, mUpPaint);
            canvas.drawRect(x - lineR, high, x + lineR, low, mUpPaint);
        }
    }
