 ...
 * @author mnlin
 */
open class BaseRecyclerViewPositionAdapter<T>(
        /**
         * 数据源
         */
        ...
        private var actionOnBindViewHolder: (View.(bean: T, position: Int) -> Unit)? = null) : RecyclerView.Adapter<BaseRecyclerViewHolder>() {

    /**
     * 第二构造器,用于array类型数据
     */
    constructor(dataResources: Array<T>, @LayoutRes layoutResId: Int = 0, listener: ((View, Int) -> Unit)? = null, onBindViewHolder: View.(bean: T, position: Int) -> Unit) : this(dataResources = dataResources.toMutableList(), layoutResId = layoutResId, listener = listener, actionOnBindViewHolder = onBindViewHolder)
    ...
    override fun onBindViewHolder(holder: BaseRecyclerViewHolder, position: Int) {
        actionOnBindViewHolder?.invoke(holder.itemView, dataResources[position], position)
    }
