 ...
 */
@RuntimePermissions
@Route(path = ARouterConst.Activity_AboutInnerActivity)
@InjectLayoutRes(layoutResId = R.layout.activity_about_inner)
@InjectActivityTitle(titleRes = R.string.label_about_inner)
class AboutInnerActivity : BaseActivity<BasePresenter<AboutInnerActivity>>(), LineMenuListener {
    ...
    override fun initData(savedInstanceState: Bundle?) {
        ...
        //如果需要更新,则开始下载
        toolbar.post { dataBean?.let { checkFinish(it) } }
    }
    ...
    override fun performSelf(lmv: LineMenuView) {
        when (lmv.position) {
            ...
            1 means "隱私政策" ->
                routeCustom(ARouterConst.Activity_CommonAgreementActivity)
                        .param(getString(R.string.activity_about_inner_secret))
                        .firstParam("${BaseApplication.app.baseWeUrl}gft_zh_tw/copyright.html")
                        .navigation()
                        .empty(comment = "協議內容")
            2 means "检查更新" ->
                    ...
                    url.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray().let {
                        it[it.size - 1].run {
                            File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), this).exists().onTrue {
                                ...
                                //显示下载进度
                                ProgressDialog.getInstance(this@AboutInnerActivity)
    ...
    private fun install(child: String) {
        ...
        if (file.exists() && file.length() > 1024) {
            ...
            ProgressDialog.getInstance(this@AboutInnerActivity).dismiss()
            ActivityUtils.finishAllActivities()
        }
    }
