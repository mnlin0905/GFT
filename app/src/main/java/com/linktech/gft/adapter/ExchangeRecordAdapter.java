...
public class ExchangeRecordAdapter extends RecyclerView.Adapter<ExchangeRecordAdapter.ViewHolder> {
    ...
    public ExchangeRecordAdapter(BaseActivity baseActivity, List<ExchangeRecord> datas, BaseRecyclerViewHolder.OnViewClickListener listener, onCancelTakeOutListener cancelTakeOutListener) {
    ...
    public ExchangeRecordAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ExchangeRecordAdapter.ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_take_out_currency_record, parent, false), listener);
    }

    @Override
    public void onBindViewHolder(ExchangeRecordAdapter.ViewHolder holder, int position) {
        ExchangeRecord bean = datas.get(position);
        holder.mTvTime.setText(bean.getCreate_time());
        holder.mTvAmount.setText(String.valueOf(bean.getAmount()));
